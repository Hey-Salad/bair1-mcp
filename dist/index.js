...
// Helpers
// ---------------------------------------------------------------------------
async function apiFetch(path, authenticated = false) {
...
function formatAqi(pm25) {
    if (pm25 <= 12)
        return { aqi: "Good (0-50)", level: "Good", advice: "Air quality is satisfactory. No health risk." };
    if (pm25 <= 35.4)
        return { aqi: "Moderate (51-100)", level: "Moderate", advice: "Acceptable. Sensitive individuals should limit prolonged outdoor exertion." };
    if (pm25 <= 55.4)
        return { aqi: "Unhealthy for Sensitive Groups (101-150)", level: "USG", advice: "People with respiratory or heart conditions, children, and older adults should reduce prolonged outdoor exertion." };
    if (pm25 <= 150.4)
        return { aqi: "Unhealthy (151-200)", level: "Unhealthy", advice: "Everyone may begin to experience health effects. Sensitive groups should avoid outdoor exertion." };
    if (pm25 <= 250.4)
        return { aqi: "Very Unhealthy (201-300)", level: "Very Unhealthy", advice: "Health alert: everyone may experience serious health effects. Avoid outdoor activities." };
...
server.tool("get_latest_reading", "Get the most recent air quality reading from a Bair1 sensor", {
...
// --- Tool: list_devices ------------------------------------------------------
server.tool("list_devices", "List all registered Bair1 air quality sensors", {}, async () => {
...
// --- Tool: get_readings ------------------------------------------------------
server.tool("get_readings", "Get historical air quality readings for a specific device", {
    ...
    const data = await apiFetch(`/api/v1/devices/${encodeURIComponent(device_id)}/readings?limit=${limit}`);
    return {
        content: [{ type: "text", text: JSON.stringify(data, null, 2) }],
    };
});
// --- Tool: get_air_quality_summary -------------------------------------------
server.tool("get_air_quality_summary", "Get a human-readable air quality summary with AQI, PM levels, and health advice", {
    ...
    if (Array.isArray(raw)) {
        if (raw.length === 0) {
            return { content: [{ type: "text", text: "No readings available." }] };
        }
        reading = raw[0];
    }
    else if (raw && typeof raw === "object" && "readings" in raw) {
        const arr = raw.readings;
...
// --- Tool: export_data -------------------------------------------------------
server.tool("export_data", "Export air quality readings as JSON for a given device and time range", {
